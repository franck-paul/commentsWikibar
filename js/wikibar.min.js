"use strict";void 0===window.dotclear&&(window.dotclear={}),dotclear.wikibar={resize_timer:void 0,previous_width:0,component:{dialog:class{title;confirmLabel;cancelLabel;fields;constructor({title:e,confirm_label:t,cancel_label:i,fields:s}={}){this.title=e,this.confirm_label=t??"Ok",this.cancel_label=i??"Cancel",this.fields=s}prompt(){if(!this.fields?.length)return Promise.resolve(null);const e=document.createElement("template"),t=this.fields.map((e=>`<p class="field">${e.html}</p>`)).join(""),i=this.title?`<h1>${this.title}</h1>`:"";e.innerHTML=((e,...t)=>e.reduce(((e,i,s)=>e+i+(t[s]??"")),""))`
